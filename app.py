"""
Interface Visual — Agente de Análise de E-Commerce
====================================================
Interface de chat construída com Streamlit, com geração automática de gráficos
...
st.title("Agente de Análise de E-Commerce")
st.caption(f"Modelo: {os.getenv('GEMINI_MODEL', 'gemini-2.5-flash')} | Banco: {os.getenv('DB_PATH', 'files/banco.db')}")
...
EXEMPLOS = {
    "Vendas e Receita": [
        ...
        "Qual foi a evolução mensal do número de pedidos?",
    ],
    "Entrega e Logística": [
    ...
    "Satisfação e Avaliações": [
        "Qual é a distribuição de notas de avaliação?",
...
# =============================================================================
# INICIALIZAÇÃO DO AGENTE (cached para não recriar a cada interação)
# =============================================================================
...
# =============================================================================
# DETECÇÃO E GERAÇÃO DE GRÁFICOS
# =============================================================================

def detectar_colunas(df: pd.DataFrame):
    """Separa colunas numéricas e categóricas do DataFrame."""
    numericas = df.select_dtypes(include="number").columns.tolist()
    categoricas = df.select_dtypes(exclude="number").columns.tolist()
    return numericas, categoricas


def contem_data(col: str) -> bool:
    """Verifica se o nome da coluna sugere uma série temporal."""
    palavras = ["mes", "ano", "data", "periodo", "month", "year", "date", "trimestre"]
    return any(p in col.lower() for p in palavras)


def gerar_grafico(df: pd.DataFrame, pergunta: str):
    """
    Gera automaticamente o gráfico mais adequado para o DataFrame retornado.
    Retorna uma figura Plotly ou None se não for possível.
    """
    ...
    # Serie temporal → grafico de linha
    ...
    # Poucas linhas com 2 colunas → grafico de barras horizontal
    ...
    # Caso geral com categorias → barras verticais
...
# Captura input — tanto do chat quanto do clique nos exemplos da sidebar
pergunta = st.chat_input("Faça uma pergunta sobre os dados do e-commerce...")
